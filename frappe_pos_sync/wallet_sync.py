...
def validate_wallet(data):
    try:
        customer = json.loads(data['scanned_nfc'])['customer'] #CUSTOMER WALLET DATA FROM Frappe POS
        attendant = json.loads(data['scanned_nfc'])['attendant'] #ATTENDANT WALLET DATA FROM Frappe POS
        receipt = data['receipt'] #RECEIPT RECORD FROM Frappe POS
        device = data['device_id'] #DEVICE ID FROM Frappe POS
