...
def generate_sales_invoice_by_date(date):
    ...
    settings = frappe.get_doc('Frappe POS Settings', 'Frappe POS Settings')
...
def sync_now():
    ...
    if settings.sales_invoice == 'By Daily Batch':
        enqueue('frappe_pos_sync.Frappe POS.generate_sales_invoice_today', timeout=2000, queue='long')
...
def get_items_with_price_list_query(device,columns=None, pos_profile=None,):
    ...
    query = """
      ...
      WHERE `tabItem`.in_frappe_pos = 1 AND `tabItem Price`.price_list= '%s' {0}""".format(condition) % (columns_str, price_list)
